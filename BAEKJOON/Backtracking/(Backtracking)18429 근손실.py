...
1 초	256 MB	1101	666	551	60.549%
문제
...
이 경우 운동 1일차에 대학원생은 중량이 3만큼 증가하지만 그와 동시에 하루에 중량이 4만큼 감소하기 때문에, 1일이 지난 이후에 중량은 499가 된다. 따라서 조건을 만족하지 못한다.
...
kit_idx = [i for i in range(N)]

cnt = 0
for ord in permutations(kit_idx):
    SamDaeMyutChino = 500
    for idx in ord:
        SamDaeMyutChino -= K
        SamDaeMyutChino += kit[idx]
        if SamDaeMyutChino < 500:
            break
    else:
        cnt += 1
