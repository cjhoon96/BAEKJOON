'''
https://www.acmicpc.net/contest/problem/666/3
3번 - 트리 디자이너 호석 80점
시간 제한	메모리 제한
1 초	1024 MB
문제
트리를 너무나 사랑하는 효성이는 트리 분재 전문가이다. 효성이가 기르는 모든 트리는 정점과 간선으로 이루어져 있다. 
...
제일 왼쪽 그림이 효성이가 가지고 있는 나무라고 하자. 가운데와 오른쪽 그림은 올바르게 전구를 고른 예시이다. 
...
즉, 이전과 수가 같아도 여전히 오름차순을 만족한다. 예를 들어 은 오름차순이지만 은 오름차순이 아니다.
...
두 번째 줄에는 번부터 번 정점에 써 있는 숫자가 공백으로 구분되어 주어진다. 각 숫자는  이상  이하의 자연수 이다.
...
queue = deque([[1]])
bulbs = set([tuple([i]) for i in range(1, n + 1)])
