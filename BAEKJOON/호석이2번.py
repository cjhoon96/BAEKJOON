'''
https://www.acmicpc.net/contest/problem/666/2
2번 - 정보 상인 호석 60점
시간 제한	메모리 제한
2 초	512 MB
문제
...
찌라시로 알 수 있는 것은, 어떤 이름을 가진 고릴라가 , , ...,  만큼의 가치가 있는 정보 k 개를 얻었다는 점이다.
...
2 Name  : 호석이가 이름이 [Name]인 고릴라에게  개의 정보를 구매한다. 
이때 고릴라가 가진 정보들 중 가장 비싼  개를 구매하며, 고릴라가 가진 정보가 개 이하이면 가진 모든 정보를 구매한다.
...
이어서  개의 줄에 걸쳐서 각 줄에 쿼리가 주어진다. 쿼리는 1이나 2로 시작한다. 
1로 시작하는 경우에는 정보를 얻은 정보 고릴라의 이름과  가 주어지며 이어서  개의 정보 가치 가 자연수로 주어진다. 
모든 는 1 이상 100,000 이하이다. 2로 시작하는 경우에는 호석이가 거래하려는 정보 고릴라의 이름과 구매하려는 정보의 개수 가 주어진다. 
...
for _ in range(q):
    ...
    if info[0] == '1':
        name = info[1]
        if name not in goril:
            goril[name] = []

        for c in info[3:]:
            goril[name].append(int(c))
        goril[name].sort()
        ...
        if name not in goril:
            continue
        elif n >= len(goril[name]):
            C += sum(goril.pop(name))
        else:
            for _ in range(n):
                C += goril[name].pop()
