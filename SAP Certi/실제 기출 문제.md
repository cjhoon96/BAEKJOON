...
47. What must you do to create a singleton class? (2 select)
...
51. A user runs an ABAP program enters an incorrect value on a selection screen and chooses executes which event
    block must send the error message in order to display the selection screen again?
    A. At Selection screen + meg E

52. Which tables could be use in this code(3) 문제에 index 1로 리드하는 코드가 주어짐, 보기 더 있었으나 답은
    아래임(주관식 아님)
    sorted table
    index table
    standard table.

53. A customer has asked that you improve performance for a small table with frequency read access. What buffering
    do you recommend?
    A. FULL
    B. PRIMARY KEY
    C. GENERIC
...
54. Advantages of defining text symbols in executable programs? (select 2)
...
55. You want to loop over an itab without copying each table row to a workarea. How can you achieve this using
    a field symbol?
    LOOP AT ITAB ASSIGINING <FS>

56. (A class is given) Super class definition is given From which components can be addressed directly from a
...
57. HOW do you create lock objects and lock modules for use the ABAP program that access the database?
    A. Use the ABAP dictionary to crate the lock objects. The lock modules are created automatically.

58. where can you defined data types that can be used throughout the system.
    A. in method
    B. in global CLASS
    C. in abap dictionary
    D. in functional module

59. You are asked to enhanced the GUI status of an Standard application. How do you identify which menu exit
...
60. You have created ZTAB and you want to create a forign key relationship to table SAPTAB . Under which condition
...
61. Which of the following can you perform in both ABAP editor and ABAP debugger?(related to breakpoint) (1)
    A. Breakpoint
    B. Watchpoint
    C. Both breakpoint and watchpoint

62. Purpose of Enqueue process?(context of the question is same but questions and answers in different format)
    LOCK MANAGEMENT

63. For which of the following purposes can you use the ABAP dictionary?
...
64. You have created the following repository objects :
