...
19. What transaction can be used to carry out modification adjustments after a system upgrade?(2) – 약자만도 나
    오고 풀 네임도 나오니 약자를 외우는 게 좋겠음
    A. SPAU
    B. SPUA
    C. SPDD
    D. SPAD
...
21. What are the tasks of databse interface? You write a program that updates a data record in the database using
...
23. which additions to the parameters statement can u use to fill the input field on the selection screen with a
    suggested value? (보기는 4~5개 되었으나 답은 아래임)
    DEFAULT
    EX: parameters p1 type i DEFAULT 10.

24. The code of an executable program does not contain any event keywords what event block does the code
    belong to?
    A. INITIALIZATION
    ...
    Which of the following Generic types can you use to define a field symbol that will be assigned to a character
    string? (select 3)
    A. Type Xsequence
    B. Type Clike
    C. Type any Table
...
25. Advantages of Open SQL? (select 2)
    A. It can be used with any DBMS
    B. All standard SQL commands can be used
    C Syntax is checked at designed time

26. You created a class by inheriting from a super class. The super class contains a public instance method
    do_something you want to redefine the method? (보기 여러 개나 답은 아래임, 1개만 고르라고 하기도 하고, 2개
    고르라고 하기도함, 1개는 a만 나왔음)
    a. Signature cannot be changed.
    b. Has to be redefine in same visibility.

27. What processes can we set when you run the code inspector?(options were somewhat different then these)(3)
    A. Background job
    B. Check variant
    C. Object set name
    D. Work process name
...
28. What type of method is generated automatically when you assign an action to a button UI element?
    OR
    What type of method is generated by WD explorer when you assign an action to a button UI element?(question
    was somewhat the same but not exactly same words were used as given above but the meaning could be depicted
    to be the same)
    A. EVENT handler
    B. Supply function
    C. Ordinary methods.
...
29. You write a report that displays mass data in a table. You decide to use tha ALV Grid control (Class
    CL_GUI_ALV_GRID) instead of a classical list display with write statements. Which of the following functions can you
    offer to the user without doing any specific programming.(2)
...
30. In which of the following source code block can you define local data objects? (select 3) –보기 더 있었으나 답
    은 아래임
    A. Static method
    B. Subroutine
    C Function module

31. Which data types are incomplete ABAP standard data types? 보기 더 있었으나 답은 아래임(주관식 아님)
    C, N, X, P

32. You need to create a piece of code that can be used by multiple programs. which of the following techniques
    for modularization does SAP recommend?(2)
...
33. In Which order you implement a new Badi?(1) 보기 더 있었으나 답은 아래임(주관식 아님)
    A. 1.Enhancement spot Implementation

34. BADI Implementation
...
37. You have been asked to a customer to develop open SQL code to convert the value of argument “arg” into the
    ABAP Dictionary specified.
    A. CAST ( arg FOR type)
...
39. screen has the following PAI flow logic:
    PROCESS AFTER INPUT
    ...
    C. A. The screen is display again without processing PBO flow logic. All fields are ready for input.
...
40. Static constructor rules?(3)(not directly asked as the same but was related to static constructor)
...
41. Your program uses the class CL_GUI_ALV_GRID to generate a classic ALV Grid Control what do you need in your
    program to react to a double clicking a row in the ALV Grid?(3)
...
42. What are new features of open SQL 7.5?(2)
    A. Full join
    B. Case expression
    C. String expression
    D. Intersections

43. What are the things that enhancements spot will manage? (3 selection)
    A. Classic BADIs
...
44. Exporting parameter named PARAM1.
    You enhance an sap standard global class by defining a post method for an sap method. The original sap method
    has an EXPORT parameter named PARAM1. what parameter does the post-method have
...
45. You call a lock module which exceptions could the lock module raise when a logical lock CANNOT be set?
    보기 더 있었으나 답은 아래임(주관식 아님)
    • FOREIGN_LOCK
    • SYSTEM_FAILURE
    48.In which event block can you overwrite the default value of a parameter field on the selection screen?
    보기 더 있었으나 답은 아래임(주관식 아님)
    INITIALIZATION

46. What must you do to create a singleton class? (2 select)
...
47. Which of the elementary data types is deep?
    A. DECFLOAT34
    B. N
    C. X
    D. STRING

48. How data types can be used throughout the system?
    A. In a Method
    B. In a function module
    C. In the ABAP dictionary.
    D. In a global class(+interface)

49. You build a dialog screen with an input field in an ABAP program.
...
50. A user runs an ABAP program enters an incorrect value on a selection screen and chooses executes which event
    block must send the error message in order to display the selection screen again?
    A. At Selection screen + meg E

51. Which tables could be use in this code(3) 문제에 index 1로 리드하는 코드가 주어짐, 보기 더 있었으나 답은
    아래임(주관식 아님)
    sorted table
    index table
    standard table.

52. A customer has asked that you improve performance for a small table with frequency read access. What buffering
    do you recommend?
    A. FULL
    B. PRIMARY KEY
    C. GENERIC
...
53. Advantages of defining text symbols in executable programs? (select 2)
...
54. You want to loop over an itab without copying each table row to a workarea. How can you achieve this using
    a field symbol?
    LOOP AT ITAB ASSIGINING <FS>

55. (A class is given) Super class definition is given From which components can be addressed directly from a
    subclass CLASS lcl_airplane?(this was given in the form of program but answers are same)
    PUBLIC, PROTECTED 존에 있는 내역 고르면 됨(클래스 구조가 주어짐)

56. HOW do you create lock objects and lock modules for use the ABAP program that access the database?
    A. Use the ABAP dictionary to crate the lock objects. The lock modules are created automatically.

57. where can you defined data types that can be used throughout the system.
    A. in method
    B. in global CLASS
    C. in abap dictionary
    D. in functional module

58. You are asked to enhanced the GUI status of an Standard application. How do you identify which menu exit
    function can you use? (select 1)
    A. It starts with a plus (+)
    B. It starts with an ampersand (&)
    C. Is starts with an asterisk(*)
    D. It starts with a dollar($)

59. You have created ZTAB and you want to create a forign key relationship to table SAPTAB . Under which condition
    is this possible ?(question was same but words were somewhat elaborated and even the answers )
    Both table fields should have same domain.

60. Which of the following can you perform in both ABAP editor and ABAP debugger?(related to breakpoint) (1)
    A. Breakpoint
    B. Watchpoint
    C. Both breakpoint and watchpoint

61. Purpose of Enqueue process?(context of the question is same but questions and answers in different format)
    LOCK MANAGEMENT

62. For which of the following purposes can you use the ABAP dictionary?
...
68. You are making changes to a program that already has a transaction code linked to it. Your colleague is testing
the transaction in your development system.
