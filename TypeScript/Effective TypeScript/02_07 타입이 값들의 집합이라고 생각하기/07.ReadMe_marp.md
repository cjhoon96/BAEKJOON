...
* ### keyof(A|B) = keyof(A)&keyof(B) ???
  ...
  `A|B` 의 경우 common property 특정 불가 => 슈퍼셋인 `never` 타입 반환???
...
* ### `extends A` <=> "A 의 부분집합"
  ...
  타입의 집합이라는 관점에서 `extends` 의 의미는  '~의 부분집합' 이라는 의미
...
* ### array 와 tuple 의 포함 관계
  ...
  Index 의 수와 각 Index 값의 Type 이 동일하면 array type 에 tuple type 의 값을 할당할 수 없다. 
