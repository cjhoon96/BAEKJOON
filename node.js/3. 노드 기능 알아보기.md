...
* ## 버퍼와 스트림 이해
  ...
  ***<u>writeme2.txt</u>***
  ...
  먼저 **createWriteStream 메서드**를 통해 **쓰기 스트림을 생성**
...
* ## 기타 fs 메서드 알아보기
  ...
  * ### fs.open(경로, 옵션, 콜백) : 
    ...
    **쓰려면 w, 읽으려면 r, 기존 파일에 추가하려면 a** 이다.
  ...
  * ### fs.rmdir(경로, 콜백) : 
    ...
    폴더 안에 파일들이 있다면 에러가 발생하므로 먼저 내부 파일을 모두 지우고 호출해야 한다. 

    node fsDelete 를 한번 더  실행하면 ENDENT 에러가 발생한다.

    존재하지 않는 파일을 지웠다는 에러이다.

  

  

  노드 8.5 버전 이후에는 createReadStream 과 createWriteStream 을 pipe 하지 않아도 파일을 복사 할 수 있다.
  ...
  ***<u>copyFile.js</u>***
  ...
  readme.txt 와 동일한 내용의 writeme4.txt 가 생성되었을 것이다.

  * 첫번째 인수로 복사할 파일 
  * 두번째 인수로 복사될 경로
  * 세번째 인수로 보사 후 실행될 콜백 함수
  ...
  ***<u>watch.js</u>***
  ...
  실행해 놓고 대상 파일인 target.txt 를 수정 삭제 등을 하면 해당내용들이 console 에 찍힌다.
...
* ## 스레드풀 알아보기
  ...
  실행된 후에는 다시 메인 스레드의 콜백 함수나 프로미스의 then 부분이 실행된다.

  이때 fs 메서드를 여러번 실행해도 백그라운드에서 동시에 처리되는데 바로 스레드풀이 있기 때문이다.
  ...
  ***<u>threadpool.js</u>***
  ...
  실행 때 마다 시간과 순서가 달라진다.
  
  스레드풀이 작업을 동시에 처리하므로 여덟 개의 작업 중에서 어느것이 먼저 처리될지 알 수 없다.
  
  하지만 하나의 규칙을 발견할 수 있다.
  
  1~4, 5~8 이 그룹으로 묶여 있으며 5~8이 시간이 더 소요된다는 것이다.
  
  이는 기본적인 스레드풀의 개수가 4개이기 때문이다.
  
  스레드 풀이 4개이므로 처음 4작업이 동시에 실행되고, 그것들이 종료되면 다음 4개의 작업이 실행된다. 
  
  
  
  스드풀을 직접 컨트롤 할 수는 없지만 개수를 조절할 수는 있다.
  
  * 윈도라면 명령 프롬프트에 SET UV_THREADPOOL_SIZE=1 
  * 맥 / 리눅스 라면 터미널에 UV_THREADPOOL_SIZE=1 
  
  을 입력한후 실행하면 
  
  작업이 순서대로 실행될 것이다.
  
  스레드 풀이 한개이므로 한번에 하나씩 밖에 처리되지 않기 때문이다.
...
* ## 이벤트 이해하기
  ...
  스트럼을 배울 때 on('data'.콜백)  또는 on('end'.콜백) 사용했다.

  바로 각 문자열에 들어있는 event 가 발생할 때 콜백함수를 호출하도록 이벤트를 등록한 것이다.

  createReadStream 같은 경우는 내부적으로 알아서 data와 end 이벤트를 호출 하지만, 직접 이벤트를 생성할 수도 있다.
  ...
  ***<u>event.js</u>*** 
  ...
  events 모듈은 event 를 생성, 제거, 관리를 가능하게 해준다.

  events 모듈로 생성한 객체는 이벤트 관리를 위한 메서드들을 가지고 있다.

  * ### on(이벤트명, 콜백) : 

    이벤트 이름과 이벤트 발생 시의 콜백을 연결한다. 

    이렇게 연결하는 동작을 이벤트 리스닝이라 한다.

    event2 처럼 이벤트 하나에 이벤트 여러개를 달아줄 수도 있다.
  ...
  * ### emit(이벤트 명) : 
    ...
    이벤트 이름을 인수로 넣으면 미리 등록해 두었던 이벤트 콜백이 실행된다.

  * ### once(이벤트명, 콜백) : 

    한 번만 실행되는 이벤트이다.

    myEvent.emit('event3') 을 두번 연속 호출했지만 콜백이 한번만 실행된다.
...
* ## 예외 처리하기
  ...
  노드의 메인 스레드는 하나뿐이므로 스레드 하나가 멈추면 프로그램 자체가 멈추게 된다.
  ...
  ***<u>error1.js</u>***
  ...
  setInterval 을 사용하여 프로세스가 멈추는지 여부를 체크하는 프로그램이다.

  에러 처리를 통해 '서버 고장' 이는 문구만 console 로 반복적으로 뜨는 것을 알 수 있다.
  ...
  ***<u>error2.js</u>***
  ...
  fs.unlink 로 존재하지도 않는 파일을 지우고 있다.

  에러가 발생하지만 노드 내장 모듈의 에러는 실행중인 프로세스를 멈추지는 않는다.

  에러 로그를 기록해두고 나중에 원인을 찾아 수정하면 된다.

  에러를 throw 를 하면 노드 프로세스가 멈춰버린다.

  따라서 throw 를 하는 경우에는 반드시 try/catch 문으로 throw한 에러를 잡아야 한다.
  ...
  ***<u>error3.js</u>***
  ...
  위와같이 프로미스의 에러는 catch 하지 않아도 알아서 처리된다.

  하지만 프로미스의 에러를 알아서 처리하는 동작은 노드 버전이 올라감에 따라 바뀔 수 있으므로 catch 를 통해 처리해주는 것을 권장한다.
  ...
  ***<u>error4.js</u>***
  ...
  process 객체에 uncaughtException 이벤트 리스너를 달았다. 처리 하지 못한 에러가 발생했을 때 이벤트 리스너가 실행되고 프로세스가 유지된다.

  이부분이 없다면 위 예제에서는 setTimeout이 실행되지 않는다. 

  실행 후 1초 만에 setInterval 에서 에러가 발생하여 프로세스가 멈추기 때문이다.

  하지만 uncaughtException 이벤트 리스너가 연결되어 있으므로 프로세스가 멈추지 않는다.

  

   try/catch 로 처리하지 못한 에러가 발생하였지만 코드가 제대로 실행된다.

  어떻게 보면 uncaughtException 이벤트 리스너로  모든 에러를 처리할 수 있을 것처럼 보인다.

  실제 uncaughtException 의 콜백 함수에 에러 발생 시 복구 작업을 하는 코드를 넣어둔 사람도 있다.

  하지만 노드 공식 문서에서는 uncaughtException 이벤트 발생 후 다음 동작이 제대로 동작하는지를 보증하지 않는다.

  즉 복구 작업 코드를 넣어 두었더라도 그것이 동작하는지 확신할 수 없다.

  

  따라서 uncaughtException 은 **단순히 에러 내용을 기록하는 정도로 사용**하고, 에러를 기록한 후 **process.exit() 으로 프로세스를 종료**하는 것이 좋다. 

  에러가 발생하는 코드를 수정하지 않는 이상, 프로세스가 실행되는 동안 에러는 계속 발생할 것이다.

   에러가 발생하는 코드를 수정하지 않는 이상, 프로세스가 실행되는 동안
...
* ## 자주  발생하는 에러들
  ...
  * ### node: command not found
    ...
    노드를 설치한 경우에도 이 에러가 발생하는 경우는 환경 변수가 제대로 설정 되지 않은 경우이다.

    환경변수에는 노드가 설치된 경로가 포함되어야한다.

  * ### Reference Error: 모듈 is not defined

    모듈을 require 했는지 여부 확인
  ...
  * ### Error: Can't set headers after they are sent
    ...
    요청에 대한 응답을 보낼 대 응답을 두 번 이상 보낸경우

    요청에 대한 응답은 한번만 보내야한다.

    응답을 보내는 메서드를 두번 이상 사용하였는지 여부 체크

  * ### FATAL ERROR: CALL_AND_RETRY_LAST Allocation failed - JavaScript heap out of memory

    코드 실행할 때 메모리가 부족하여 스크립트가 정상 작동하지 않는 경우

    코드가 잘못 되었을 확률이 높으므로 코드를 점검해 보아야한다.

    만약 코드가 정상인 경우 노드의 메모리를 늘릴 수 있다.

    노드를 실행할 때 node --max-old-space-size=4096 파일명 과 같은 명령어를 사용하면 된다.

  * ### UnhandledPromiseRejectionWarning: Unhandled promis rejection

    프로미스 사용시 catch 메서드를 붙이지 않으면 발생한다.

    항상 catch 를 붙여 에러가 나는 상황에 대비하여야 한다.

  * ### EADDRINUSE 포트 번호

    해당 포트 번호에 이미 다른 프로세스가 연결되어있다.

    그 프로세스는 노드 프로세스일 수도 있고 다른 프로그램일 수도 있다.

    그 프로세스를 종료하거나 다른 포트 번호를 사용하여야 한다.
  ...
  * ### EACCES 또는 EPERM
    ...
    파일 / 폴더 수정/삭제/생성 권한 확인이 필요

    맥이나 리눅스 운영체제라면 명령어 앞에 sudo 를 붙이는 것도 방법이다.

  * ### EJSONPARSE

    package.json 등의 JSON 파일에 문법 오류가 있을 때 발생

    자바스크립트 객체와는 형식이 조금 다르니 쉼표같은 게 빠지거나 추가되지는 않았는지 확인

  * ### ECONNREFUSED

    요청을 보냈으나 연결이 성립하지 않을 때 발생

    요청을 받는 서버의 주소가 올바른지, 꺼져있지는 않은지 확인이 필요
  ...
  * ### ETIMEOUT
    ...
    역시 요청을 받는 서버의 상태를 점검해 봐야한다.

  * ### ENDENT: no such file or directory

    지정한 폴더나 파일이 존재하지 않는 경우

    맥이나 리눅스 운영체제에서는 대소문자도 구별하므로 확인해봐야한다.
